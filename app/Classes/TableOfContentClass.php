...
class TableOfContentClass
{
    ...
    private function _make_contents_callback($match)
    {
        ...
        $this->contents[] = "\t" . '
				<li' . ($_is_mark ? ' itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"' : '') . '>
					<a rel="nofollow" href="' . $opt->page_url . '#' . $anchor . '">' . $this->count . '. ' . $cont_elem_txt . '</a>
        ...
        if ($opt->anchor_link) {
            $tag_txt = '<a rel="nofollow" class="anchlink" href="#' . $anchor . '">' . $opt->anchor_link . '</a> ' . $tag_txt;
        }
        
        // anchor type: 'a' or 'id'
        if ($opt->anchor_type === 'a') {
            $new_el = '<a class="anchor" name="' . $anchor . '"></a>' . "\n<$tag $attrs>$tag_txt</$tag>";
        } else {
