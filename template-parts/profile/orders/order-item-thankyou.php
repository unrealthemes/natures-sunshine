    ...
    if ( $partner_ids ) : 
        foreach ( (array)$partner_ids as $partner_id => $full_name ) : 
            ...
            foreach ( $order->get_items() as $item_id => $item ) :
                ...
                $product_in_join_cart = get_post_meta( $order->get_id(), $field_key, true );
                if ( $product_in_join_cart ) :
                    $pv = (float)str_replace(",", ".", get_post_meta( $item->get_product_id(), '_pv', true ) );
...
<li class="orders-results__list-item orders-result">
    <div class="orders-result__info <?php echo esc_attr($line_class); ?>">
        ...
        <div class="orders-result__info-body collapse-panel" style="max-height: max-content;">
            <div class="order">

                <div class="order__row">
                    <div class="order__row-title active">
                        <?php _e('Recipient data', 'natures-sunshine'); ?>
                    </div>
                    <div class="order__row-content" style="max-height: max-content;">
                        <ul class="order-info">
